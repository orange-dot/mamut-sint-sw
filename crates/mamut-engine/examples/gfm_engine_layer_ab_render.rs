...
fn main() -> std::io::Result<()> {
    ...
    for (slug, patch_source) in AB_PATCHES {
        ...
        render_wav(
            format!("{OUTPUT_DIR}/ab_base_{slug}.wav"),
            patch.clone(),
            GfmLayerMode::Disabled,
            "baseline",
        )?;
        render_wav(
            format!("{OUTPUT_DIR}/ab_gfm_{slug}.wav"),
            patch,
...
fn render_wav(
    ...
    let gesture_selection = select_gfm_program_for_patch(&patch);
    let Some(gesture_program_id) = gesture_selection.program_id else {
        return Err(std::io::Error::other(format!(
            "patch did not select a GFM gesture program: {patch_name}"
        )));
    };
    ...
    if matches!(mode, GfmLayerMode::Enabled { .. })
        && layer_selection.program_id != Some(gesture_program_id)
    {
        return Err(std::io::Error::other(format!(
            "GFM layer selected {:?}, expected {:?} for {patch_name}",
            layer_selection.program_id,
            Some(gesture_program_id)
        )));
    }

    let note_on_events = layer_note_on_events(gesture_program_id);
    ...
    while rendered < frames {
        ...
        if rendered == 0 {
            ...
            let note_events = layer_note_off_events(gesture_program_id, frame_offset);
            process_block(
                &mut engine,
                frame_count,
                &note_events,
    ...
    if let Some(diagnostics) = snapshot.diagnostics {
        println!(
            "{} mode={} patch=\"{}\" gesture_program={:?} active={:?} scores=({:.4},{:.4},{:.4}) rms={:.4} peak={:.4} max_ruptures={} health={:?}",
            path.display(),
            mode_label,
            patch_name,
            gesture_program_id,
        ...
        println!(
            "{} mode={} patch=\"{}\" gesture_program={:?} active={:?} scores=({:.4},{:.4},{:.4}) rms={:.4} peak={:.4}",
            path.display(),
            mode_label,
            patch_name,
            gesture_program_id,
            snapshot.active_program_id,
            gesture_selection.horizont_score,
            gesture_selection.pec_score,
            gesture_selection.baklja_score,
            rms,
            peak_abs
        );
    }
