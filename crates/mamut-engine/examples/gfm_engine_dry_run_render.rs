...
fn main() -> std::io::Result<()> {
    ...
    for program_id in GfmProgramId::ALL_PERFORMANCE {
        render_wav(
            format!("{OUTPUT_DIR}/{}", engine_wav_file_name(program_id)),
            program_id,
        )?;
    }
...
fn render_wav(path: impl AsRef<Path>, program_id: GfmProgramId) -> std::io::Result<()> {
    ...
    if !finite {
        return Err(std::io::Error::other(
            "GFM engine dry-run render produced non-finite output",
        ));
    }
...
fn engine_wav_file_name(program_id: GfmProgramId) -> &'static str {
    match program_id {
        GfmProgramId::HorizontPerformance => "engine_gfm_horizont.wav",
        GfmProgramId::PecPerformance => "engine_gfm_pec.wav",
        GfmProgramId::BakljaPerformance => "engine_gfm_baklja.wav",
    }
}
